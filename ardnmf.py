...
class mvnmf:
    ...
    def factorize(self, K=None, a_W=None, b_W=None, a_H=None, b_H=None, tie_lambda=True, max_iter=200, compute_variational_lower_bound=True, variational_lower_bound_min_rate=1e-4, rng=numpy.random.RandomState(1000)):
        """
        ...
        if tie_lambda:
            E_Lambda_W=self.rng.gamma(shape=self.a_W, scale=1/self.b_W, size=(self.K,1))
            E_Lambda_H=E_Lambda_W
        else:
            E_Lambda_W=self.rng.gamma(shape=self.a_W, scale=1/self.b_W, size=(self.K,1))
            E_Lambda_H=self.rng.gamma(shape=self.a_H, scale=1/self.b_H, size=(self.K,1))
        
        #compute A_W, B_W, A_H, and B_H using prior expoential distribution(1,lambda)
        A_W=1 #numpy.ones(shape=(self.M,self.K),dtype=float)
        ONE_M_1=numpy.ones(shape=(self.M,1),dtype=float)
        B_W=numpy.dot(ONE_M_1, numpy.transpose(E_Lambda_W)) + self.tol
        #print B_W
        A_H=1 #numpy.ones(shape=(self.K,self.N),dtype=float)
        ONE_1_N=numpy.ones(shape=(1,self.N),dtype=float)
        B_H=numpy.dot(E_Lambda_H, ONE_1_N) + self.tol
        #print B_H
        ...
        ONE_N_1=numpy.ones(shape=(self.N,1),dtype=float)
        ...
        while num_iter<=self.max_iter:
            ...
            A_W=1 + Sigma_W
            B_W=numpy.dot(ONE_M_1, numpy.transpose(E_Lambda_W)) + ONE_M_N.dot(E_H.transpose())
            #compute E_W
            E_W=A_W/B_W 
            A_H=1 + Sigma_H.transpose()
            B_H=numpy.dot(E_Lambda_H, ONE_1_N) + E_W.transpose().dot(ONE_M_N)
            #compute E_H
            E_H=A_H/B_H
            ...
            if tie_lambda:
                A_Lambda_W=self.a_W+self.M+self.N
                B_Lambda_W=self.b_W+numpy.dot(E_W.transpose(),ONE_M_1) + numpy.dot(E_H,ONE_N_1)
                E_Lambda_W=A_Lambda_W/B_Lambda_W
                A_Lambda_H=A_Lambda_W
                B_Lambda_H=B_Lambda_W
                E_Lambda_H=E_Lambda_W
            else:
                A_Lambda_W=self.a_W+self.M
                B_Lambda_W=self.b_W+numpy.dot(E_W.transpose(),ONE_M_1)
                E_Lambda_W=A_Lambda_W/B_Lambda_W
                A_Lambda_H=self.a_H+self.N
                B_Lambda_H=b_H+numpy.dot(E_H,ONE_N_1)
